 ...
 *
 * This is the URL where users are redirected for authentication (login page).
 * Configure via NEXT_PUBLIC_AUTH_APP_URL environment variable.
 *
 * @returns The base URL for the auth app (e.g., https://auth.example.workers.dev)
 */
export const getAuthAppUrl = (): string => {
	const baseUrl = process.env.NEXT_PUBLIC_AUTH_APP_URL;
	...
	if (!baseUrl.startsWith("http://") && !baseUrl.startsWith("https://")) {
		throw new Error(
			`NEXT_PUBLIC_AUTH_APP_URL must include the protocol (https://). Got: "${baseUrl}"`,
		);
	}
